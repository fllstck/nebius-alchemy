...
> = globalThis.__ALCHEMY_RUNTIME__
  ...
  reconcile: Effect.fn('Nebius.vpc.v1.Allocation.reconcile')(function* ({ id, news, output, session, olds }) {
    news = news || {}
    ...
    if (
     ...
     ||
      ...
      Factory.labelsDrifted(allocation.metadata?.labels, news.labels, olds?.labels)
    ) {
      ...
      allocation = yield* vpcGrpcService.allocation.update({
        metadata: {
          ...
          resourceVersion: allocation.metadata!.resourceVersion.toString(),
          labels,
        },
        spec: desired,
      })
