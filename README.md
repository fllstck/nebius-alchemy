...
- **`Nebius.mk8s.NodeGroup`** — Worker node groups (the *minimal CPU arm*: `version`, `fixedNodeCount` and the whole `template` — OS, hardware, boot disk, network interfaces, service account, cloud-init). The parent is the **cluster**, not the project, so `parentId` is required and there is no project fallback; deleting the cluster cascades to its node groups. `template.os` and `resources.preset` are **not** validated against a list — which images a group may use depends on the cluster's Kubernetes version *and* the platform, so `Nebius.mk8s.action.GetNodeGroupCompatibilityMatrix({ clusterKubernetesVersion, platform })` is the authority. `template.bootDisk.sizeGibibytes` is required (the platform's 64 GiB floor is enforced; a smaller disk hangs provisioning before cloud-init) and `template.networkInterfaces[].publicIpAddress` can only be turned **on** (`false` is a plan-time error). Omit `version` to inherit the cluster's resolved one. A `template.cloudInitUserData` change does **not** rewrite existing nodes (the proto rolls the group out; the solutions library warns the field is sticky) — recreate nodes in a maintenance window. **Not yet run against real infra**: the gated live test exists and asserts `RUNNING` plus a no-drift forced reconcile, but costs a real VM
