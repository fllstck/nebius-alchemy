...
describe('attributes: int64 fields arrive as decimal strings', () => {
   ...
   */
  test('Disk reports its int64 attributes as strings', () => {
    ...
    const attrs = ResourceUtils.toFriendlyAttributes<Record<string, unknown>>({
      rawResource: NebiusDiskSchema.Disk.fromJSON({
        spec: { sizeGibibytes: '64', blockSizeBytes: '4096', type: 'NETWORK_SSD' },
        metadata: { id: 'computedisk-1', name: 'disk', parentId: 'project-1' },
      }),
      resourceSchema: NebiusDiskSchema.Disk,
    })
    expect(attrs.sizeGibibytes).toBe('64')
    expect(attrs.blockSizeBytes).toBe('4096')
  })
})
